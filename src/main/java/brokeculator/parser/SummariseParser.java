    ...
    public static Command parseInput(String userInput) {
        ...
        if (userInputAsArray.length == 3) {
            if (userInputAsArray[1].equals("/from")) {
                ...
                if (beginIndex < 0) {
                    return new InvalidCommand("Start index cannot be negative integer");
                }
                // endIndex == -1;
                return new SummariseCommand(beginIndex, endIndex);
                ...
                if (endIndex < 0) {
                    return new InvalidCommand("End index cannot be negative integer");
                }
                return new SummariseCommand(beginIndex, endIndex);
            }
        ...
        if (userInputAsArray.length == 5) {
            ...
            try {
                beginIndex = Integer.parseInt(userInputAsArray[2]);
                endIndex = Integer.parseInt(userInputAsArray[4]);
            } catch (NumberFormatException e) {
                return new InvalidCommand("Start and end index cannot be non-integer");
            }
            if (beginIndex < 0 || endIndex < 0) {
                return new InvalidCommand("Start and end index cannot be negative integer");
            }
